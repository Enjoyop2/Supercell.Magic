...
namespace Supercell.Magic.Titan.Util
{
	public class LogicLongToCodeConverterUtil
	{
		...
		{
			m_hashTag = hashTag;
			m_conversionChars = conversionChars;
		}
		...
		public string ToCode(LogicLong logicLong)
		{
			...
			if (highValue < 256)
			{
				return m_hashTag + Convert(((long)logicLong.GetLowerInt() << 8) | (uint)highValue);
			}
		...
		{
			long id = 0;
			int conversionCharsCount = m_conversionChars.Length;
			int codeCharsCount = code.Length;

			for (int i = 0; i < codeCharsCount; i++)
			{
				int charIndex = m_conversionChars.IndexOf(code[i]);
		...
		private string Convert(long value)
		{
			...
			if (value > -1)
			{
				...
				for (int i = 11; i >= 0; i--)
				{
					code[i] = m_conversionChars[(int)(value % conversionCharsCount)];
