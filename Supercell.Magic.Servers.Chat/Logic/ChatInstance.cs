...
namespace Supercell.Magic.Servers.Chat.Logic
{
	public class ChatInstance
	{
		...
		public ChatInstance()
		{
			m_sessions = new Dictionary<long, ChatSession>(ChatInstance.CAPACITY);
		}
		...
		{
			if (!m_sessions.Remove(session.Id))
				throw new Exception("ChatInstance.remove: session is not in instance");
		}

		public bool IsFull()
			=> m_sessions.Count >= ChatInstance.CAPACITY;
		...
		public void PublishMessage(LogicClientAvatar logicClientAvatar, string message)
		{
			...
			foreach (ChatSession session in m_sessions.Values)
			{
