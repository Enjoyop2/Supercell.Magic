...
namespace Supercell.Magic.Tools.Client
{
	public static class ResourceManager
	{
		...
		public static byte[] LoadAsset(string file)
			=> ResourceManager.DecompressAsset(file, File.ReadAllBytes("assets/" + file));

		public static int GetContentVersion()
			=> ResourceManager.m_version[2];
		...
		private class DownloadTask
		{
			...
			{
				m_task = new WebClient().DownloadDataTaskAsync(string.Format("{0}/{1}/{2}", contentUrl, fingerprintSha, fileName));
				FileName = fileName;
			}
