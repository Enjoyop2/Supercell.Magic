...
namespace Supercell.Magic.Tools.Client.Network
{
	public class Messaging
	{
		...
		public Messaging()
		{
			...
			m_receiveBuffer = new SocketBuffer(2048);
			m_messageFactory = LogicMagicMessageFactory.Instance;
			m_receiveQueue = new ConcurrentQueue<PiranhaMessage>();
			m_sendQueue = new ConcurrentQueue<PiranhaMessage>();

			m_scramblerSeed = Environment.TickCount;
			InitEncrypters();
		}
		...
		public void Connect(string host, int port)
		{
			...
			if (!m_socket.ConnectAsync(connectEventArgs))
		...
		private void OnConnect(object sender, SocketAsyncEventArgs args)
		{
			if (args.SocketError == SocketError.Success)
			{
				...
				if (!m_socket.ReceiveAsync(receiveEventArgs))
					OnReceive(null, args);
		...
		private void OnReceive(object sender, SocketAsyncEventArgs args)
		{
			do
			{
				...
				{
					Debugger.Warning(string.Format("Messaging.onReceive: connection with the server {0} has been lost ({1})", m_socket.RemoteEndPoint, args.SocketError));
					break;
				}

				m_receiveBuffer.Write(args.Buffer, args.BytesTransferred);
				OnReceive();

			} while (!m_socket.ReceiveAsync(args));
		}
		...
		private void OnReceive()
		{
			...
			{
				int read = OnReceive(m_receiveBuffer.GetBuffer(), length);
				if (read <= 0)
					break;
			...
			if (length >= Messaging.PACKET_HEADER_LENGTH)
			{
				...
				if (length >= Messaging.PACKET_HEADER_LENGTH + messageLength)
				{
					...
					if (m_receiveEncrypter != null)
					{
						...
						m_receiveEncrypter.Decrypt(encryptedBytes, encodingBytes, encodingLength);
					}
					else
					{
					...
					if (piranhaMessage != null)
					{
						...
						{
							m_receiveQueue.Enqueue(piranhaMessage);
						}
						else
						{
							...
							{
								case 1:
									ScrambleNonceUsingMersenneTwister(nonce);
									break;
								default:
									ScrambleNonceUsingDefaultMethod(nonce);
									break;
							}
							...
							InitEncrypters(new string(nonceChars));
						}
					}
					else
					{
		...
		public bool TryDequeueReceiveMessage(out PiranhaMessage message)
			=> m_receiveQueue.TryDequeue(out message);

		public void Send(PiranhaMessage piranhaMessage)
		{
			if (!piranhaMessage.IsServerToClientMessage())
				m_sendQueue.Enqueue(piranhaMessage);
		}

		public void SendQueue()
		{
			while (m_sendQueue.TryDequeue(out PiranhaMessage piranhaMessage))
			{
				...
				if (m_sendEncrypter != null)
				{
					...
					m_sendEncrypter.Encrypt(encodingBytes, encryptedBytes, encodingLength);
				}
				else
				{
				...
				Send(stream, encryptedLength + Messaging.PACKET_HEADER_LENGTH);
			}
		}
		...
		private void OnSend(object sender, SocketAsyncEventArgs args)
		{
			...
			{
				m_socket.Close();
				Debugger.Warning(string.Format("Messaging.onSend: connection with the server {0} has been lost ({1})", m_socket.RemoteEndPoint, args.SocketError));
			}
