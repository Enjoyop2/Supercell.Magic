...
namespace Supercell.Magic.Tools.Client.Libs.ZLib
{
	...
	public class CRC32
	{
		...
		public int Crc32Result
		{
			get
			{
				return unchecked((int)~m_register);
			}
		}

		public int GetCrc32(Stream input)
			=> GetCrc32AndCopy(input, null);
		...
		public int GetCrc32AndCopy(Stream input, Stream output)
		{
			...
			unchecked
			{
				...
				return (int)~m_register;
			}
		}


		public int ComputeCrc32(int W, byte B)
			=> m_InternalComputeCrc32((uint)W, B);
		...
		public void SlurpBlock(byte[] block, int offset, int count)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (reverseBits)
				{
					uint temp = (m_register >> 24) ^ b;
					m_register = (m_register << 8) ^ crc32Table[temp];
				}
				else
				{
					uint temp = (m_register & 0x000000FF) ^ b;
					m_register = (m_register >> 8) ^ crc32Table[temp];
				}
			}
		...
		public void UpdateCRC(byte b, int n)
		{
			while (n-- > 0)
			{
				...
				{
					uint temp = (m_register >> 24) ^ b;
					m_register = (m_register << 8) ^ crc32Table[temp >= 0
																				 ? temp
																				 : temp + 256];
				}
				else
		...
		private void GenerateLookupTable()
		{
			...
			unchecked
			{
				...
				do
				{
					...
					if (reverseBits)
					{
						crc32Table[CRC32.ReverseBits(i)] = CRC32.ReverseBits(dwCrc);
					}
					else
					{
						crc32Table[i] = dwCrc;
	...
	public class CrcCalculatorStream : Stream, IDisposable
	{
			...
			(bool leaveOpen, long length, Stream stream, CRC32 crc32)
		{
			...
			m_Crc32 = crc32 ?? new CRC32();
			m_lengthLimit = length;
			LeaveOpen = leaveOpen;
		}
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			if (m_lengthLimit != CrcCalculatorStream.UnsetLengthLimit)
			{
				if (m_Crc32.TotalBytesRead >= m_lengthLimit)
				{
					return 0; // EOF
				}

				long bytesRemaining = m_lengthLimit - m_Crc32.TotalBytesRead;
			...
			int n = m_innerStream.Read(buffer, offset, bytesToRead);
			if (n > 0)
			{
				m_Crc32.SlurpBlock(buffer, offset, n);
			}
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			if (count > 0)
			{
				m_Crc32.SlurpBlock(buffer, offset, count);
			}
