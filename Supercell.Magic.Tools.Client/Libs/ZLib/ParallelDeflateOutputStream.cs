...
namespace Supercell.Magic.Tools.Client.Libs.ZLib
{
	...
	public class ParallelDeflateOutputStream : Stream
	{
		...
		{
			m_toWrite = new Queue<int>();
			m_toFill = new Queue<int>();
			m_pool = new List<WorkItem>();
			int nTasks = ParallelDeflateOutputStream.BufferPairsPerCore * Environment.ProcessorCount;
			nTasks = Math.Min(nTasks, m_maxBufferPairs);
			for (int i = 0; i < nTasks; i++)
			{
				m_pool.Add(new WorkItem(m_bufferSize, m_compressLevel, Strategy, i));
				m_toFill.Enqueue(i);
			}

			m_newlyCompressedBlob = new AutoResetEvent(false);
			m_runningCrc = new CRC32();
			m_currentlyFilling = -1;
			m_lastFilled = -1;
			m_lastWritten = -1;
			m_latestCompressed = -1;
		}
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			if (m_pendingException != null)
			{
				...
				Exception pe = m_pendingException;
			...
			if (!m_firstWriteDone)
			{
				...
				m_InitializePoolOfWorkItems();
				m_firstWriteDone = true;
			}
			...
			do
			{
				...
				int ix = -1;
				if (m_currentlyFilling >= 0)
				{
					ix = m_currentlyFilling;
				...
				else
				{
					...
					TraceOutput(TraceBits.WriteTake,
									 ...
									 ix, m_lastFilled);
					++m_lastFilled; // TODO: consider rollover?
				}

				WorkItem workitem = m_pool[ix];

				int limit = workitem.buffer.Length - workitem.inputBytesAvailable > count
					? count
					: workitem.buffer.Length - workitem.inputBytesAvailable;

				workitem.ordinal = m_lastFilled;
				...
				if (workitem.inputBytesAvailable == workitem.buffer.Length)
				{
					...
					if (!ThreadPool.QueueUserWorkItem(m_DeflateOne, workitem))
					{
						throw new Exception("Cannot enqueue workitem");
					}

					m_currentlyFilling = -1; // will get a new buffer next time
				}
				else
				{
					m_currentlyFilling = ix;
				}
		...
		private void m_FlushFinish()
		{
			...
			int rc = compressor.InitializeDeflate(m_compressLevel, false);
			...
			Crc32 = m_runningCrc.Crc32Result;
		}
		...
		private void m_Flush(bool lastInput)
		{
			...
			if (m_currentlyFilling >= 0)
			{
				WorkItem workitem = m_pool[m_currentlyFilling];
				m_DeflateOne(workitem);
				m_currentlyFilling = -1; // get a new buffer next Write()
			}
		...
		public void Reset(Stream stream)
		{
			...
			TraceOutput(TraceBits.Session, "Reset {0:X8} firstDone({1})", GetHashCode(), m_firstWriteDone);
			...
			m_toWrite.Clear();
			m_toFill.Clear();
			foreach (WorkItem workitem in m_pool)
			{
				m_toFill.Enqueue(workitem.index);
				workitem.ordinal = -1;
			}

			m_firstWriteDone = false;
			BytesProcessed = 0L;
			m_runningCrc = new CRC32();
			m_isClosed = false;
			m_currentlyFilling = -1;
		...
		private void EmitPendingBuffers(bool doAll, bool mustWait)
		{
			...
			do
			{
				...
				do
				{
					if (Monitor.TryEnter(m_toWrite, millisecondsToWait))
					{
						nextToWrite = -1;
						try
						{
							...
							{
								nextToWrite = m_toWrite.Dequeue();
							}
						}
						finally
						{
							Monitor.Exit(m_toWrite);
						}

						if (nextToWrite >= 0)
						{
							WorkItem workitem = m_pool[nextToWrite];
							if (workitem.ordinal != m_lastWritten + 1)
							{
								// out of order. requeue and try again.
								TraceOutput(TraceBits.EmitSkip,
												 "Emit     skip     wi({0}) ord({1}) lw({2}) fs({3})",
												 workitem.index,
												 workitem.ordinal, m_lastWritten,
												 firstSkip);

								lock (m_toWrite)
								{
									m_toWrite.Enqueue(nextToWrite);
								}
							...
							m_runningCrc.Combine(workitem.crc, workitem.inputBytesAvailable);
							...
							m_lastWritten = workitem.ordinal;
							m_toFill.Enqueue(workitem.index);
			...
			} while (doAll && m_lastWritten != m_latestCompressed);
		...
		private void m_DeflateOne(object wi)
		{
			...
			try
			{
				...
				lock (m_latestLock)
				{
					if (workitem.ordinal > m_latestCompressed)
					{
						m_latestCompressed = workitem.ordinal;
					}
				}

				lock (m_toWrite)
				{
					m_toWrite.Enqueue(workitem.index);
				}

				m_newlyCompressedBlob.Set();
			}
