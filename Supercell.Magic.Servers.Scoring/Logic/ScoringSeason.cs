...
namespace Supercell.Magic.Servers.Scoring.Logic
{
	public class ScoringSeason : SeasonDocument
	{
		...
		public void Init()
		{
			m_allianceRankingList = new Dictionary<long, AllianceRankingEntry>[2];

			m_allianceRankingList[0] = new Dictionary<long, AllianceRankingEntry>();
			m_allianceRankingList[1] = new Dictionary<long, AllianceRankingEntry>();

			m_avatarRankingList = new Dictionary<long, AvatarRankingEntry>();
			m_avatarDuelRankingList = new Dictionary<long, AvatarDuelRankingEntry>();
		...
		public async Task Update()
		{
			...
			Task t1 = UpdateAllianceRankingList(0);
			Task t2 = UpdateAllianceRankingList(1);
			Task t3 = UpdateAvatarRankingList();
			Task t4 = UpdateAvatarDuelRankingList();
			...
			{
				foreach (AllianceRankingEntry rankingEntry in m_allianceRankingList[0].Values)
					rankingEntry.SetPreviousOrder(rankingEntry.GetOrder());
				foreach (AllianceRankingEntry rankingEntry in m_allianceRankingList[1].Values)
					rankingEntry.SetPreviousOrder(rankingEntry.GetOrder());
				foreach (AvatarRankingEntry rankingEntry in m_avatarRankingList.Values)
					rankingEntry.SetPreviousOrder(rankingEntry.GetOrder());
				foreach (AvatarDuelRankingEntry rankingEntry in m_avatarDuelRankingList.Values)
					rankingEntry.SetPreviousOrder(rankingEntry.GetOrder());

				NextCheckTime = DateTime.UtcNow.Date.AddDays(1);
			}

			await ServerScoring.SeasonDatabase.Update(Id, CouchbaseDocument.Save(this));
		}
		...
		{
			Dictionary<long, AllianceRankingEntry> prevAllianceRankingDictionary = m_allianceRankingList[villageType];
			...
			LogicArrayList<AllianceRankingEntry> allianceRankingList = AllianceRankingList[villageType];
			...
			if (result.Success)
			{
				...
				m_allianceRankingList[villageType] = allianceRankingDictionary;
			}
			else
			{
		...
		{
			Dictionary<long, AvatarRankingEntry> prevAvatarRankingDictionary = m_avatarRankingList;
			...
			LogicArrayList<AvatarRankingEntry> avatarRankingList = AvatarRankingList;
			...
			if (result.Success)
			{
				...
				m_avatarRankingList = avatarRankingDictionary;
			}
			else
			{
		...
		{
			Dictionary<long, AvatarDuelRankingEntry> prevAvatarDuelRankingDictionary = m_avatarDuelRankingList;
			...
			LogicArrayList<AvatarDuelRankingEntry> avatarDuelRankingList = AvatarDuelRankingList;
			...
			if (result.Success)
			{
				...
				m_avatarDuelRankingList = avatarDuelRankingDictionary;
			}
			else
			{
