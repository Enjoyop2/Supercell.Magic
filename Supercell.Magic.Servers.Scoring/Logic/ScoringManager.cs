...
namespace Supercell.Magic.Servers.Scoring.Logic
{
	public static class ScoringManager
	{
		...
		public static LogicArrayList<int> GetDiamondPrizes()
			=> ScoringManager.m_diamondPrizes;

		public static int GetNextEndTimeSeconds()
			=> LogicMath.Max((int)(ScoringManager.m_currentSeason.EndTime.Subtract(DateTime.UtcNow).TotalSeconds + 0.99d), 0);

		public static int GetSeasonYear()
			=> ScoringManager.m_currentSeason.Year;

		public static int GetSeasonMonth()
			=> ScoringManager.m_currentSeason.Month;
		...
		public static LogicArrayList<AllianceRankingEntry> GetAllianceRankingList(int villageType, LogicLong allianceId)
			=> ScoringManager.GetRankingList<AllianceRankingEntry>(ScoringManager.m_currentSeason.AllianceRankingList[villageType], allianceId);

		public static LogicArrayList<AllianceRankingEntry> GetLastSeasonAllianceRankingList(int villageType, LogicLong allianceId)
			=> ScoringManager.GetRankingList<AllianceRankingEntry>(ScoringManager.m_lastSeason?.AllianceRankingList[villageType], allianceId);

		public static LogicArrayList<AvatarRankingEntry> GetAvatarRankingList(LogicLong avatarId)
			=> ScoringManager.GetRankingList<AvatarRankingEntry>(ScoringManager.m_currentSeason.AvatarRankingList, avatarId);

		public static LogicArrayList<AvatarRankingEntry> GetLastSeasonAvatarRankingList(LogicLong avatarId)
			=> ScoringManager.GetRankingList<AvatarRankingEntry>(ScoringManager.m_lastSeason?.AvatarRankingList, avatarId);

		public static LogicArrayList<AvatarDuelRankingEntry> GetAvatarDuelRankingList(LogicLong avatarId)
			=> ScoringManager.GetRankingList<AvatarDuelRankingEntry>(ScoringManager.m_currentSeason.AvatarDuelRankingList, avatarId);

		public static LogicArrayList<AvatarDuelRankingEntry> GetLastSeasonAvatarDuelRankingList(LogicLong avatarId)
			=> ScoringManager.GetRankingList<AvatarDuelRankingEntry>(ScoringManager.m_lastSeason?.AvatarDuelRankingList, avatarId);
