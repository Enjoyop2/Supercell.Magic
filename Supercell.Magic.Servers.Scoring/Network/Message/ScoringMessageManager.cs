...
namespace Supercell.Magic.Servers.Scoring.Network.Message
{
	public class ScoringMessageManager : ServerMessageManager
	{
		...
		public override void OnReceiveRequestMessage(ServerRequestMessage message)
		{
			...
			{
				_00A0((LeaderboardRequestMessage)message);
			}
		}

		private static void _00A0(LeaderboardRequestMessage message)
		{
			LogicArrayList<AvatarRankingEntry> avatarRankingEntrylist = new LogicArrayList<AvatarRankingEntry>(message.Count);
			LogicArrayList<AvatarDuelRankingEntry> avatarDuelRankingEntryList = new LogicArrayList<AvatarDuelRankingEntry>(message.Count);
			LogicArrayList<AvatarRankingEntry> avatarRankingEntry = ScoringManager.GetAvatarRankingList(null);
			LogicArrayList<AvatarDuelRankingEntry> avatarDuelRankingEntry = ScoringManager.GetAvatarDuelRankingList(null);

			for (int i = 0; i < LogicMath.Min(avatarRankingEntry.Size(), message.Count); i++)
			{
				avatarRankingEntrylist.Add(avatarRankingEntry[i]);
			}

			for (int j = 0; j < LogicMath.Min(avatarDuelRankingEntry.Size(), message.Count); j++)
			{
				avatarDuelRankingEntryList.Add(avatarDuelRankingEntry[j]);
			}

			ServerRequestManager.SendResponse(new LeaderboardResponseMessage
			{
				Success = true,
				MainLeaderboard = avatarRankingEntrylist,
				SecondaryLeaderboard = avatarDuelRankingEntryList
			}, message);
		}
