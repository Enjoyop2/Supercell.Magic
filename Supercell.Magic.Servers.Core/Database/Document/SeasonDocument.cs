...
namespace Supercell.Magic.Servers.Core.Database.Document
{
	public class SeasonDocument : CouchbaseDocument
	{
		...
		public LogicArrayList<AllianceRankingEntry>[] AllianceRankingList
		{
			get; private set;
		}
		public LogicArrayList<AvatarRankingEntry> AvatarRankingList
		{
			get; private set;
		}
		public LogicArrayList<AvatarDuelRankingEntry> AvatarDuelRankingList
		{
			get; private set;
		}
		...
		{
			AllianceRankingList = new LogicArrayList<AllianceRankingEntry>[2];
			AllianceRankingList[0] = new LogicArrayList<AllianceRankingEntry>(SeasonDocument.RANKING_LIST_SIZE);
			AllianceRankingList[1] = new LogicArrayList<AllianceRankingEntry>(SeasonDocument.RANKING_LIST_SIZE);
			AvatarRankingList = new LogicArrayList<AvatarRankingEntry>(SeasonDocument.RANKING_LIST_SIZE);
			AvatarDuelRankingList = new LogicArrayList<AvatarDuelRankingEntry>(SeasonDocument.RANKING_LIST_SIZE);
		}
		...
		protected sealed override void Encode(ByteStream stream)
		{
			for (int i = 0; i < 2; i++)
			{
				LogicArrayList<AllianceRankingEntry> allianceRankingList = AllianceRankingList[i];
			...
			stream.WriteVInt(AvatarRankingList.Size());

			for (int i = 0; i < AvatarRankingList.Size(); i++)
				AvatarRankingList[i].Encode(stream);

			stream.WriteVInt(AvatarDuelRankingList.Size());

			for (int i = 0; i < AvatarDuelRankingList.Size(); i++)
				AvatarDuelRankingList[i].Encode(stream);
		}
		...
		protected sealed override void Save(LogicJSONObject jsonObject)
		{
			...
			for (int i = 0; i < AvatarRankingList.Size(); i++)
			{
				avatarRankingArray.Add(AvatarRankingList[i].Save());
			}

			for (int i = 0; i < AvatarDuelRankingList.Size(); i++)
			{
				avatarDuelRankingArray.Add(AvatarDuelRankingList[i].Save());
			}
			...
			jsonObject.Put(SeasonDocument.JSON_ATTRIBUTE_NEXT_CHECK_TIME, new LogicJSONString(NextCheckTime.ToString("O")));
		}
		...
		protected sealed override void Load(LogicJSONObject jsonObject)
		{
			...
			NextCheckTime = DateTime.Parse(jsonObject.GetJSONString(SeasonDocument.JSON_ATTRIBUTE_NEXT_CHECK_TIME).GetStringValue());
		}
	}
}