...
namespace Supercell.Magic.Servers.Core.Database
{
	public class RedisDatabase
	{
		...
		public RedisDatabase(string name)
		{
			...
			EndPoint[] endPoints = m_redis.GetEndPoints();
			...
			{
				byte[] hash = luaScript.Load(m_redis.GetServer(endPoints[i])).Hash;

				if (hash.Length != 20)
					throw new Exception("RedisDatabase.ctor: hash length != 20");

				if (m_removeIfScript != null)
				{
					for (int j = 0; j < 20; j++)
					{
						if (m_removeIfScript[j] != hash[j])
							throw new Exception("RedisDatabase.ctor: hash mismatch");
					}
				}
				else
				{
					m_removeIfScript = hash;
				}
			}
		}

		public Task<bool> Exists(long id)
			=> m_database.KeyExistsAsync(id.ToString());

		public Task<bool> Delete(long id)
			=> m_database.KeyDeleteAsync(id.ToString());

		public Task<bool> Set(long id, string value)
			=> m_database.StringSetAsync(id.ToString(), value);

		public Task<RedisValue> GetSet(long id, string value)
			=> m_database.StringGetSetAsync(id.ToString(), value);

		public Task<RedisValue> Get(long id)
			=> m_database.StringGetAsync(id.ToString());

		public Task<RedisValue[]> Get(RedisKey[] ids)
			=> m_database.StringGetAsync(ids);

		public async void DeleteIfEquals(long id, string value)
		{
			await m_database.ScriptEvaluateAsync(m_removeIfScript, new RedisKey[]
			{
				id.ToString()
			}, new RedisValue[]
			{
