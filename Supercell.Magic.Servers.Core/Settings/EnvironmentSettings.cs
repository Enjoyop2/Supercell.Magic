...
namespace Supercell.Magic.Servers.Core.Settings
{
	public static class EnvironmentSettings
	{
		...
		public static ServerConnectionEntry[][] Servers
		{
			get; private set;
		}
		public static CouchbaseSettings Couchbase
		{
			get; private set;
		}
		public static RedisSettings Redis
		{
			get; private set;
		}
		...
		public static int HigherIdCounterSize
		{
			get; private set;
		}
		public static string[] SupportedAppVersions
		{
			get; private set;
		}
		...
		public static string[] DeveloperIPs
		{
			get; private set;
		}
		...
		public static bool IsDeveloperIP(string ip)
			=> Array.IndexOf(EnvironmentSettings.DeveloperIPs, ip) != -1;

		public static bool IsStageServer()
			=> EnvironmentSettings.Environment.StartsWith("stage");

		public static bool IsIntegrationServer()
			=> EnvironmentSettings.Environment.StartsWith("integration");

		public static bool IsProductionServer()
			=> EnvironmentSettings.Environment.Equals("prod");
		...
		{
			public LogicArrayList<RedisDatabaseEntry> Databases
			{
				get;
			}
			...
			public RedisSettings(LogicJSONObject jsonObject)
			{
				...
				Databases = new LogicArrayList<RedisDatabaseEntry>(databases.Size());

				for (int i = 0; i < databases.Size(); i++)
				{
					Databases.Add(new RedisDatabaseEntry(databases.GetJSONObject(i)));
				}
			}
			...
			public LogicJSONObject Save()
			{
				...
				for (int i = 0; i < Databases.Size(); i++)
				{
					databaseArray.Add(Databases[i].Save());
				}
			...
			public struct RedisDatabaseEntry
			{
				...
				{
					Name = jsonObject.GetJSONString("name").GetStringValue();
					ConnectionString = jsonObject.GetJSONString("connectionString").GetStringValue();
				}
				...
				public LogicJSONObject Save()
				{
					...
					jsonObject.Put("connectionString", new LogicJSONString(ConnectionString));
		...
		public struct ServerSettings
		{
			...
			public bool ContentValidationModeEnabled
			{
				get;
			}
			...
			{
				ContentValidationModeEnabled = jsonObject.GetJSONBoolean("contentValidationModeEnabled").IsTrue();
				Proxy = new ProxySettings(jsonObject.GetJSONObject("proxy"));
				Admin = new AdminSettings(jsonObject.GetJSONObject("admin"));
			}
			...
			public LogicJSONObject Save()
			{
				...
				jsonObject.Put("contentValidationModeEnabled", new LogicJSONBoolean(ContentValidationModeEnabled));
				jsonObject.Put("proxy", Proxy.Save());
				jsonObject.Put("admin", Admin.Save());
			...
			public struct ProxySettings
			{
				...
				{
					SessionCapacity = LogicMath.Clamp(jsonObject.GetJSONNumber("sessionCapacity").GetIntValue(), 0, 16000);
				}

				public LogicJSONObject Save()
				{
					LogicJSONObject jsonObject = new LogicJSONObject();
					jsonObject.Put("sessionCapacity", new LogicJSONNumber(SessionCapacity));
					return jsonObject;
				}
			}
			...
			{
				public LogicArrayList<string> PresetLevelFiles
				{
					get;
				}
				...
				public AdminSettings(LogicJSONObject jsonObject)
				{
					...
					{
						PresetLevelFiles = new LogicArrayList<string>(presetLevelFileArray.Size());

						for (int i = 0; i < presetLevelFileArray.Size(); i++)
						{
							PresetLevelFiles.Add(presetLevelFileArray.GetJSONString(i).GetStringValue());
						}
					}
					else
					{
						PresetLevelFiles = null;
					}
				}
		...
		public struct ServerConnectionEntry
		{
			...
			public ServerConnectionEntry(LogicJSONString jsonString)
			{
				...
				ServerIP = connectionString[0];
				ServerPort = int.Parse(connectionString[1]);
			}
			...
			{
				return new LogicJSONString(string.Format("{0}:{1}", ServerIP, ServerPort));
			}
		}

		public struct CouchbaseSettings
		{
			public LogicArrayList<CouchbaseServerEntry> Servers
			{
				get;
			}
			public LogicArrayList<CouchbaseBucketEntry> Buckets
			{
				get;
			}
			...
			public CouchbaseSettings(LogicJSONObject jsonObject)
			{
				...
				Servers = new LogicArrayList<CouchbaseServerEntry>(serverArray.Size());
				Buckets = new LogicArrayList<CouchbaseBucketEntry>(bucketArray.Size());

				for (int i = 0; i < serverArray.Size(); i++)
				{
					Servers.Add(new CouchbaseServerEntry(serverArray.GetJSONObject(i)));
				}
			...
			private int GetBucketIdx(string name)
			{
				...
				{
					if (Buckets[i].Name.Equals(name, StringComparison.InvariantCultureIgnoreCase))
					{
						return i;
					}
				}
			...
			{
				int index = GetBucketIdx(name);

				if (index != -1)
				{
					bucketEntry = Buckets[index];
					serverEntry = Servers[bucketEntry.ServerIndex];
			...
			public LogicJSONObject Save()
			{
				...
				{
					serverArray.Add(Servers[i].Save());
				}

				for (int i = 0; i < Buckets.Size(); i++)
				{
					bucketArray.Add(Buckets[i].Save());
				}
		...
		public struct CouchbaseServerEntry
		{
			...
			public CouchbaseServerEntry(LogicJSONObject jsonObject)
			{
				...
				Hosts = new Uri[hostArray.Size()];

				for (int i = 0; i < hostArray.Size(); i++)
				{
					Hosts[i] = new Uri("http://" + hostArray.GetJSONString(i).GetStringValue());
				}

				Username = jsonObject.GetJSONString("username").GetStringValue();
				Password = jsonObject.GetJSONString("password").GetStringValue();
			}
			...
			public LogicJSONObject Save()
			{
				...
				{
					hostArray.Add(new LogicJSONString(Hosts[i].ToString()));
				}

				jsonObject.Put("hosts", hostArray);
				jsonObject.Put("username", new LogicJSONString(Username));
				jsonObject.Put("password", new LogicJSONString(Password));
		...
		public class CouchbaseBucketEntry
		{
			...
			public CouchbaseBucketEntry(LogicJSONString str)
			{
				...
				Name = name[0];
				ServerIndex = int.Parse(name[1]);
			}

			public LogicJSONString Save()
			{
				return new LogicJSONString(string.Format("{0}:{1}", Name, ServerIndex));
			}
		}
	}
}