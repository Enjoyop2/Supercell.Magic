	...
	public class UserController : Controller
	{
		...
		public async Task<JObject> Get(long id, [FromQuery(Name = "pass")] string passToken)
		{
			...
			if (accountDocument == null || gameDocument == null)
				return this.BuildResponse(HttpStatusCode.InternalServerError);
			if (GetUserRole() <= UserRole.NULL && passToken != accountDocument.PassToken)
			...
			if (online)
			{
				...
				if (GetUserRole() >= UserRole.TESTER)
				{
		...
		{
			if (Request.Headers.TryGetValue("token", out StringValues token) && AuthManager.TryGetSession(token, out SessionEntry session))
		...
		{
			public AdministrationCommandArgEntry[] Args
			{
				get; set;
			}
