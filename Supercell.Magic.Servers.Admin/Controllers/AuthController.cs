...
namespace Supercell.Magic.Servers.Admin.Controllers
{
	...
	public class AuthController : Controller
	{
		...
		{
			if (Request.Headers.TryGetValue(AuthController.TOKEN_ATTRIBUTE, out StringValues prevToken) && AuthManager.IsOpenSession(prevToken))
		...
		{
			if (!AuthManager.CloseSession(Request.Headers[AuthController.TOKEN_ATTRIBUTE]))
				return this.BuildResponse(HttpStatusCode.NotFound);
		...
		{
			if (AuthManager.TryGetSession(Request.Headers[AuthController.TOKEN_ATTRIBUTE], out SessionEntry entry))
