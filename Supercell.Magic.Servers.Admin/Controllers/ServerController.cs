...
namespace Supercell.Magic.Servers.Admin.Controllers
{
	...
	public class ServerController : Controller
	{
		...
		public JObject Index()
			=> this.BuildResponse(HttpStatusCode.OK).AddAttribute("servers", ServerManager.Save());

		[HttpGet("startMaintenance")]
		public JObject StartMaintenance()
		{
			if ((ServerStatus.Status == ServerStatusType.NORMAL || ServerStatus.Status == ServerStatusType.COOLDOWN_AFTER_MAINTENANCE) && Request.Query.TryGetValue("duration", out StringValues durationStr))
			{
		...
		{
			if (ServerStatus.Status == ServerStatusType.MAINTENANCE && Request.Query.TryGetValue("duration", out StringValues durationStr))
			{
