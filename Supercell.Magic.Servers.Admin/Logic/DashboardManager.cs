...
namespace Supercell.Magic.Servers.Admin.Logic
{
	public static class DashboardManager
	{
		...
		public static int DailyActiveUsers
		{
			get; set;
		}
		...
		private static void Update()
		{
			while (true)
			{
				...
				if (totalUsersResult.Success)
					DashboardManager.TotalUsers = (int)totalUsersResult.Rows[0]["$1"];
				if (dailyActiveUsersResult.Success)
					DashboardManager.DailyActiveUsers = (int)dailyActiveUsersResult.Rows[0]["$1"];
				if (newUsersResult.Success)
					DashboardManager.NewUsers = (int)newUsersResult.Rows[0]["$1"];
