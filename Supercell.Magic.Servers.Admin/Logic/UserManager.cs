...
namespace Supercell.Magic.Servers.Admin.Logic
{
	public static class UserManager
	{
		...
		public static string GetPresetLevel()
			=> UserManager.m_presetLevels[UserManager.m_presetRandom.Rand(UserManager.m_presetLevels.Size())];
		...
		public static async Task<JArray> Search(string name, int expLevel, int score, string allianceName)
		{
			...
			if (result.Success)
			{
				...
				for (int i = 0; i < result.Rows.Count; i++)
				{
					...
					if (original.ContainsKey("alliance_name"))
						jObject.Add("allianceName", original["alliance_name"]);
