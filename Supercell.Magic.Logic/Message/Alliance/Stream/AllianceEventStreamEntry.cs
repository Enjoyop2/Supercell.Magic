...
namespace Supercell.Magic.Logic.Message.Alliance.Stream
{
	public class AllianceEventStreamEntry : StreamEntry
	{
		...
		public override void Decode(ByteStream stream)
		{
			...
			m_eventType = (AllianceEventStreamEntryType)stream.ReadInt();
			m_eventAvatarId = stream.ReadLong();
			m_eventAvatarName = stream.ReadString(900000);
		}
		...
		public override void Encode(ByteStream stream)
		{
			...
			stream.WriteInt((int)m_eventType);
			stream.WriteLong(m_eventAvatarId);
			stream.WriteString(m_eventAvatarName);
		}
		...
		{
			m_eventAvatarName = message;
		}

		public override StreamEntryType GetStreamEntryType()
			=> StreamEntryType.ALLIANCE_EVENT;
		...
		public override void Load(LogicJSONObject jsonObject)
		{
			...
			m_eventType = (AllianceEventStreamEntryType)jsonObject.GetJSONNumber("event_type").GetIntValue();
			...
			{
				m_eventAvatarId = new LogicLong(eventAvatarIdHighObject.GetIntValue(), eventAvatarIdLowObject.GetIntValue());

				LogicJSONString eventAvatarNameObject = jsonObject.GetJSONString("event_avatar_name");

				if (eventAvatarNameObject != null)
				{
					m_eventAvatarName = eventAvatarNameObject.GetStringValue();
				}
			}
		}
		...
		public override void Save(LogicJSONObject jsonObject)
		{
			...
			jsonObject.Put("event_type", new LogicJSONNumber((int)m_eventType));

			if (!m_eventAvatarId.IsZero())
			{
				jsonObject.Put("event_avatar_id_high", new LogicJSONNumber(m_eventAvatarId.GetHigherInt()));
				jsonObject.Put("event_avatar_id_low", new LogicJSONNumber(m_eventAvatarId.GetLowerInt()));
				jsonObject.Put("event_avatar_name", new LogicJSONString(m_eventAvatarName));
			}
		}
	}
