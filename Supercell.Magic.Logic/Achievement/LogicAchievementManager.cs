	...
	public class LogicAchievementManager
	{
			...
			if (homeOwnerAvatar != null)
			{
				if (homeOwnerAvatar.IsClientAvatar())
				{
					...
					for (int i = 0; i < dataTable.GetItemCount(); i++)
					{
						...
						{
							RefreshAchievementProgress(clientAvatar, achievementData, clientAvatar.GetAchievementProgress(achievementData) + 1);
						}
					}
				}
			}
				...
				for (int i = 0, progress = 0; i < dataTable.GetItemCount(); i++, progress = 0)
				{
					...
					switch (achievementData.GetActionType())
					{
						...
						case LogicAchievementData.ACTION_TYPE_UPGRADE:
							progress = m_level.GetGameObjectManager().GetHighestBuildingLevel(achievementData.GetBuildingData()) + 1;
							break;
						...
						case LogicAchievementData.ACTION_TYPE_UNIT_UNLOCK:
							progress = achievementData.GetCharacterData()
													  .IsUnlockedForBarrackLevel(
														  LogicMath.Max(m_level.GetComponentManagerAt(achievementData.GetVillageType()).GetMaxBarrackLevel(), 0))
								? 1
								: 0;
							break;
						...
						case LogicAchievementData.ACTION_TYPE_GEAR_UP:
							progress = m_level.GetGameObjectManager().GetGearUpBuildingCount();
							break;
						case LogicAchievementData.ACTION_TYPE_REPAIR_BUILDING:
							...
							if (achievementLevels.Size() > 0)
							{
								...
								{
									if (!IsBuildingUnlocked(achievementLevels[j].GetBuildingData()))
									{
										break;
									}
					...
					RefreshAchievementProgress(playerAvatar, achievementData, progress);
				}
			}
		}
			...
			if (m_level.GetState() != 5)
			{
				...
				if (currentValue < tmp)
				{
					...
					if (playerAvatar == avatar)
					{
						...
						{
							m_level.GetGameListener().AchievementCompleted(data);
						}
						else
						{
							m_level.GetGameListener().AchievementProgress(data);
						}
					}
				}
			}
		...
		public void AlianceUnitDonated(LogicCharacterData data)
		{
			if (data != null)
			{
				...
				for (int i = 0; i < dataTable.GetItemCount(); i++)
				{
					...
					{
						RefreshAchievementProgress(playerAvatar, achievementData, playerAvatar.GetAchievementProgress(achievementData) + data.GetHousingSpace());
					}
				}
			}
		}
		...
		public void PvpAttackWon()
		{
			...
			for (int i = 0; i < table.GetItemCount(); i++)
			{
				...
				{
					RefreshAchievementProgress(playerAvatar, data, playerAvatar.GetAchievementProgress(data) + 1);
				}
			}
		}
		...
		public void PvpDefenseWon()
		{
			if (m_level.GetHomeOwnerAvatar().IsClientAvatar())
			{
				...
				for (int i = 0; i < table.GetItemCount(); i++)
				{
					...
					{
						RefreshAchievementProgress(homeOwnerAvatar, data, homeOwnerAvatar.GetAchievementProgress(data) + 1);
					}
				}
			}
		}
		...
		public void IncreaseWarStars(int stars)
		{
			...
			for (int i = 0; i < table.GetItemCount(); i++)
			{
				...
				{
					RefreshAchievementProgress(playerAvatar, data, playerAvatar.GetAchievementProgress(data) + stars);
				}
			}
		}
		...
		public void IncreaseLoot(LogicResourceData resourceData, int count)
		{
			...
			for (int i = 0; i < table.GetItemCount(); i++)
			{
				...
				{
					RefreshAchievementProgress(playerAvatar, data, playerAvatar.GetAchievementProgress(data) + count);
				}
			}
		}
		...
		public bool IsBuildingUnlocked(LogicBuildingData buildingData)
		{
			LogicArrayList<LogicGameObject> gameObjects =
				m_level.GetGameObjectManagerAt(buildingData.GetVillageType()).GetGameObjects(LogicGameObjectType.BUILDING);
