...
namespace Supercell.Magic.Logic.Level
{
	public sealed class LogicTileMap
	{
		...
		public LogicTileMap(int x, int y)
		{
			m_sizeX = x;
			m_sizeY = y;
			m_tiles = new LogicTile[x * y];

			for (int i = 0; i < m_tiles.Length; i++)
			{
				m_tiles[i] = new LogicTile((byte)(i % x), (byte)(i / x));
			}
		}
		...
		{
			if (m_sizeX > (uint)x && m_sizeY > (uint)y)
				return m_tiles[x + m_sizeX * y];
			return null;
		}
		...
		public LogicPathFinder GetPathFinder()
		{
			if (m_pathFinder == null)
			{
				...
				{
					m_pathFinder = new LogicPathFinderNew(this);
				}
				else
				{
					m_pathFinder = new LogicPathFinderOld(this);
				}
			}
		...
		{
			LogicTile tile = GetTile(x / 2, y / 2);
		...
		{
			LogicTile tile = GetTile(tileX, tileY);
			...
			if (gameObject.IsStaticObject())
			{
				...
				if (tileX >= 0 && tileY >= 0)
				{
					...
					for (int i = 0; i < sizeY; i++)
					{
						...
						{
							m_tiles[(tileX + j) + m_sizeX * (tileY + i)].AddGameObject(gameObject);
						}
					}
			...
			if (gameObject.IsStaticObject())
			{
				...
				if (tileX >= 0 && tileY >= 0)
				{
					...
					for (int i = 0; i < sizeY; i++)
					{
						for (int j = 0; j < sizeX; j++)
						{
							m_tiles[(prevTileX + j) + m_sizeX * (prevTileY + i)].RemoveGameObject(gameObject);
						}
					}
			...
			if (gameObject.IsStaticObject())
			{
				...
				if (tileX >= 0 && tileY >= 0)
				{
					...
					for (int i = 0; i < sizeY; i++)
					{
						for (int j = 0; j < sizeX; j++)
						{
							m_tiles[(tileX + j) + m_sizeX * (tileY + i)].RemoveGameObject(gameObject);
						}
					}
		...
		{
			if (enabled && !m_roomEnabled)
			{
				m_roomEnabled = true;
				UpdateRoomIndices();
			}

			m_roomEnabled = enabled;
		}
			...
			if (gameObject.IsStaticObject())
			{
				...
				if (tileX >= 0 && tileY >= 0)
				{
					...
					for (int i = 0; i < sizeY; i++)
					{
						for (int j = 0; j < sizeX; j++)
						{
							LogicTile tile = GetTile(tileX + j, tileY + i);
		...
		public void UpdateRoomIndices()
		{
			if (m_roomEnabled)
			{
				int tileCount = m_tiles.Length;

				for (int i = 0; i < tileCount; i++)
				{
					m_tiles[i].SetRoomIdx(m_tiles[i].IsFullyNotPassable() ? (short)-1 : (short)0);
				}
		...
		{
			if (m_tiles[tileIndex].GetRoomIdx() == 0)
			{
				int fillCount = 1;
				int tileCount = m_sizeX * m_sizeY;

				if (m_openNodes == null)
				{
					m_openNodes = new int[tileCount];
				}

				m_openNodes[0] = tileIndex;

				for (int i = 0; i < fillCount; i++)
				{
					int openNode = m_openNodes[i];

					if (m_tiles[openNode].GetRoomIdx() == 0)
					{
						int openNodeY = openNode / m_sizeX;
						int openNodeX = openNode % m_sizeX;
						int openNodeIdx = openNodeY * m_sizeX;
						...
						do
						{
							if (openNodeX-- <= 0)
								break;
							tile = m_tiles[openNodeStart--];
						} while (tile.GetRoomIdx() == 0);

						while (openNodeEndX < m_sizeX)
						{
							if (m_tiles[openNodeEnd++].GetRoomIdx() != 0)
							{
								break;
							}
						...
						if (openNodeX <= openNodeEndX)
						{
							...
							do
							{
								if (tileX >= 0 && tileX < m_sizeX)
								{
									if (tileX > openNodeX && tileX < openNodeEndX)
									{
										m_tiles[tileX + openNodeIdx].SetRoomIdx(roomIdx);
									}

									if (openNodeY > 0)
									{
										if (m_tiles[tileX + openNodeIdx - m_sizeX].GetRoomIdx() == 0)
										{
											m_openNodes[fillCount++] = tileX + openNodeIdx - m_sizeX;
										}
									}

									if (openNodeY < m_sizeY - 1)
									{
										if (m_tiles[tileX + openNodeIdx + m_sizeX].GetRoomIdx() == 0)
										{
											m_openNodes[fillCount++] = tileX + openNodeIdx + m_sizeX;
		...
		public bool IsValidAttackPos(int x, int y)
		{
			for (int i = 0, posX = x - 1; i < 2; i++, posX++)
			{
				...
				{
					LogicTile tile = GetTile(posX + i, posY + j);
		...
		public bool GetNearestPassablePosition(int x, int y, LogicVector2 output, int radius)
		{
			...
			while (startTileX <= endTileX)
			{
				...
				{
					if (IsPassablePathFinder(startTileX, startTileY))
					{
		...
		public bool GetWallInPassableLine(int startX, int startY, int endX, int endY, LogicVector2 output)
		{
			...
			tile = GetTile(endTileX, endTileY);
		...
		public bool GetPassablePositionInLine(int startX, int startY, int endX, int endY, int radius, LogicVector2 output)
		{
			...
			{
				if (IsPassablePathFinder(x >> 8, y >> 8))
				{
					output.m_x = x;
					output.m_y = y;
			...
			if (IsPassablePathFinder(endX >> 8, endY >> 8))
			{
				output.m_x = endX;
				output.m_y = endY;
