...
namespace Supercell.Magic.Logic.Command.Debug
{
	public sealed class LogicDebugCommand : LogicCommand
	{
		...
		public override int Execute(LogicLevel level)
		{
			...
			switch (m_debugAction)
			{
				...
				case LogicDebugActionType.LOAD_LEVEL:
					LogicDebugUtil.LoadDebugJSON(level, m_debugString);
					break;
				case LogicDebugActionType.UPGRADE_BUILDING:
					LogicDebugUtil.SetBuildingUpgradeLevel(level, m_globalId, m_debugInt, level.GetVillageType());
					break;
				case LogicDebugActionType.UPGRADE_BUILDINGS:
					if (m_globalIds != null && m_intArrayArg2 != null)
					{
						for (int i = 0, count = LogicMath.Min(m_globalIds.Size(), m_intArrayArg2.Size()); i < count; i++)
						{
							LogicDebugUtil.SetBuildingUpgradeLevel(level, m_globalIds[i], m_intArrayArg2[i], level.GetVillageType());
						}
					}
		...
		{
			m_debugAction = (LogicDebugActionType)stream.ReadInt();
			m_debugString = stream.ReadString(900000);
			m_globalId = stream.ReadInt();
			m_debugInt = stream.ReadInt();

			int intArrayArg1Size = stream.ReadInt();

			if (intArrayArg1Size > 0)
			{
				m_globalIds.EnsureCapacity(intArrayArg1Size);

				for (int i = 0; i < intArrayArg1Size; i++)
				{
					m_globalIds[i] = stream.ReadInt();
				}
			}
			...
			{
				m_intArrayArg2.EnsureCapacity(intArrayArg2Size);

				for (int i = 0; i < intArrayArg2Size; i++)
				{
					m_intArrayArg2[i] = stream.ReadInt();
				}
			}
		...
		{
			encoder.WriteInt((int)m_debugAction);
			encoder.WriteString(m_debugString);
			encoder.WriteInt(m_globalId);
			encoder.WriteInt(m_debugInt);

			if (m_globalIds != null)
			{
				encoder.WriteInt(m_globalIds.Size());

				for (int i = 0; i < m_globalIds.Size(); i++)
				{
					encoder.WriteInt(m_globalIds[i]);
				}
			}
			else
			{
			...
			if (m_intArrayArg2 != null)
			{
				encoder.WriteInt(m_intArrayArg2.Size());

				for (int i = 0; i < m_intArrayArg2.Size(); i++)
				{
					encoder.WriteInt(m_intArrayArg2[i]);
				}
			}
			else
			{
		...
		public override LogicJSONObject GetJSONForReplay()
		{
			...
			jsonObject.Put("dA", new LogicJSONNumber((int)m_debugAction));
			jsonObject.Put("gi", new LogicJSONNumber(m_globalId));
			jsonObject.Put("int", new LogicJSONNumber(m_debugInt));

			if (!string.IsNullOrEmpty(m_debugString))
			{
				Debugger.Warning("DebugCommand m_pString not saved for replay");
			}

			if (m_globalIds != null && m_globalIds.Size() > 0)
			{
		...
		public override void LoadFromJSON(LogicJSONObject jsonRoot)
		{
			...
			{
				m_debugAction = (LogicDebugActionType)debugActionObject.GetIntValue();
			}
			...
			{
				m_globalId = globalIdNumber.GetIntValue();
			}
			...
			{
				m_debugInt = intArgNumber.GetIntValue();
			}
		}
