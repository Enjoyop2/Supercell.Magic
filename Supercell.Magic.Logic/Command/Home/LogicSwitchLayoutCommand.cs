...
namespace Supercell.Magic.Logic.Command.Home
{
	public class LogicSwitchLayoutCommand : LogicCommand
	{
		...
		public override void Decode(ByteStream stream)
		{
			...
			m_layoutType = stream.ReadInt();

			if (m_layoutId == 6 || m_layoutId == 7)
			{
				m_layoutId = -1;
			}
		...
		public override int Execute(LogicLevel level)
		{
			if (m_layoutId != -1)
			{
				if (level.GetTownHallLevel(level.GetVillageType()) >= level.GetRequiredTownHallLevelForLayout(m_layoutId, -1))
				{
					if ((m_layoutId & 0xFFFFFFFE) != 6)
					{
						if (m_layoutType != 0)
						{
							if (m_layoutId != 1 && m_layoutId != 4 && m_layoutId != 5)
							{
								return -5;
							}
						}
					...
					for (int i = 0; i < gameObjects.Size(); i++)
					{
						...
						LogicVector2 position = gameObject.GetPositionLayout(m_layoutId, false);
					...
					if (m_layoutType != 0)
					{
						...
						level.SetActiveWarLayout(m_layoutId);
					}
					else
					{
						level.SetActiveLayout(m_layoutId, level.GetVillageType());
		...
		public override LogicCommandType GetCommandType()
			=> LogicCommandType.CHANGE_LAYOUT;
	}
}