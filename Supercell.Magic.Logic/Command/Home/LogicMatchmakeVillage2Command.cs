...
namespace Supercell.Magic.Logic.Command.Battle
{
	public sealed class LogicMatchmakeVillage2Command : LogicCommand
	{
		...
		public override void Decode(ByteStream stream)
		{
			...
			m_buyResourceCount = stream.ReadVInt();
		...
		public override void Encode(ChecksumEncoder encoder)
		{
			...
			encoder.WriteVInt(m_buyResourceCount);

			base.Encode(encoder);
		}

		public override LogicCommandType GetCommandType()
			=> LogicCommandType.MATCHMAKE_VILLAGE2;
		...
		public override int Execute(LogicLevel level)
		{
			if (level.GetVillageType() == 1)
			{
				if (level.IsUnitsTrainedVillage2())
				{
					if (level.GetState() == 1)
					{
						if (m_buyResourceCount > 0)
						{
							LogicClientAvatar playerAvatar = level.GetPlayerAvatar();

							if (playerAvatar.GetUnusedResourceCap(LogicDataTables.GetGold2Data()) < m_buyResourceCount)
							{
								return -1;
							}

							int buyResourceCost = LogicGamePlayUtil.GetResourceDiamondCost(m_buyResourceCount, LogicDataTables.GetGold2Data());
							...
							playerAvatar.GetChangeListener().DiamondPurchaseMade(5, LogicDataTables.GetGold2Data().GetGlobalID(), m_buyResourceCount, buyResourceCost,
																				 level.GetVillageType());
							playerAvatar.CommodityCountChangeHelper(0, LogicDataTables.GetGold2Data(), m_buyResourceCount);
						}
