...
namespace Supercell.Magic.Logic.Command.Home
{
	public sealed class LogicMoveAllBuildingsEditModeCommand : LogicCommand
	{
		...
		public override LogicCommandType GetCommandType()
			=> LogicCommandType.MOVE_ALL_BUILDINGS_EDIT_MODE;
		...
		public override int Execute(LogicLevel level)
		{
			...
			for (int i = 0; i < gameObjects.Size(); i++)
			{
				...
				{
					int newX = editModePosition.m_x + m_x;
					int newY = editModePosition.m_y + m_y;
			...
			for (int i = 0; i < gameObjects.Size(); i++)
			{
				...
				if (editModePosition.m_x != -1 && editModePosition.m_y != -1)
				{
					gameObject.SetPositionLayoutXY(editModePosition.m_x + m_x, editModePosition.m_y + m_y, m_layoutId, true);
				}
			}
