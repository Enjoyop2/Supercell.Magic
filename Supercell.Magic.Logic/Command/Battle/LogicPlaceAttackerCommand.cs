...
namespace Supercell.Magic.Logic.Command.Battle
{
	public sealed class LogicPlaceAttackerCommand : LogicCommand
	{
		...
		public override void Decode(ByteStream stream)
		{
			m_x = stream.ReadInt();
			m_y = stream.ReadInt();
			m_data = (LogicCharacterData)ByteStreamHelper.ReadDataReference(stream, DataType.CHARACTER);
		...
		public override void LoadFromJSON(LogicJSONObject jsonRoot)
		{
			...
			if (dataNumber != null)
			{
				m_data = (LogicCharacterData)LogicDataTables.GetDataById(dataNumber.GetIntValue(), DataType.CHARACTER);
			}
