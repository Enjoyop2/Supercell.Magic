	...
	public class LogicStartingHomeVillage2Command : LogicServerCommand
	{
		...
		public override int Execute(LogicLevel level)
		{
			...
			if (playerAvatar != null)
			{
				...
				{
					LogicJSONObject jsonObject = level.GetGameListener().ParseCompressedHomeJSON(m_compressedHomeJSON, m_compressedHomeJSON.Length);

					level.SetLoadingVillageType(1);

					LoadGameObjectsJsonArray(level, jsonObject.GetJSONArray("buildings2"), 1);
					LoadGameObjectsJsonArray(level, jsonObject.GetJSONArray("obstacles2"), 1);
					LoadGameObjectsJsonArray(level, jsonObject.GetJSONArray("traps2"), 1);
					LoadGameObjectsJsonArray(level, jsonObject.GetJSONArray("decos2"), 1);
		...
		public override LogicCommandType GetCommandType()
			=> LogicCommandType.STARTING_HOME_VILLAGE2;

		public void SetData(byte[] compressedHomeJSON)
		{
			m_compressedHomeJSON = compressedHomeJSON;
		}
