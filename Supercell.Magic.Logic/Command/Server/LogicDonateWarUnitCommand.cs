...
namespace Supercell.Magic.Logic.Command.Server
{
	public class LogicDonateWarUnitCommand : LogicServerCommand
	{
		...
		public override void Decode(ByteStream stream)
		{
			...
			m_unitData = ByteStreamHelper.ReadDataReference(stream) as LogicCombatItemData;
					...
					if (m_unitData.GetVillageType() == 0)
					{
						if (!m_unitData.IsDonationDisabled())
						{
							...
							if (m_quickDonate)
							{
								...
								{
									playerAvatar.GetChangeListener().WarDonateFailed(m_unitData, upgLevel, m_streamId, m_quickDonate);
									return 0;
								}
							}
							else
							...
							playerAvatar.GetChangeListener().WarDonateOk(m_unitData, upgLevel, m_streamId, m_quickDonate);
							...
							if (m_quickDonate)
							{
								...
								if (level.GetState() == 1 || level.GetState() == 3 &&
									m_unitData.GetCombatItemType() == LogicCombatItemData.COMBAT_ITEM_TYPE_CHARACTER)
								{
									// TODO: Finish this.
								}
							}
		...
		public override LogicCommandType GetCommandType()
			=> LogicCommandType.DONATE_WAR_UNIT;
	}
}