...
namespace Supercell.Magic.Logic.Command
{
	public class LogicCommandManager
	{
		...
		{
			m_level = level;
			m_commandList = new LogicArrayList<LogicCommand>();
		}
		...
		public void Decode(ByteStream stream)
		{
			...
			{
				m_commandList.Add(LogicCommandManager.DecodeCommand(stream));
			}
		...
		public void Encode(ChecksumEncoder encoder)
		{
			...
			encoder.WriteInt(m_commandList.Size());

			for (int i = 0; i < m_commandList.Size(); i++)
			{
				LogicCommand command = m_commandList[i];
		...
		public void SubTick()
		{
			int subTick = m_level.GetLogicTime().GetTick();
			...
			for (int i = 0; i < m_commandList.Size(); i++)
			{
				...
				if (command.GetExecuteSubTick() == subTick)
				{
					if (IsCommandAllowedInCurrentState(command))
					{
						if (command.Execute(m_level) == 0)
						{
							m_listener?.CommandExecuted(command);
							m_level.GetGameMode().GetReplay()?.RecordCommand(command);
						}
