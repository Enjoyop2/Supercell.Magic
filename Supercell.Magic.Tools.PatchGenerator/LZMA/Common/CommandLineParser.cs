...
namespace SevenZip.CommandLineParser
{
	...
	public class Parser
	{
		...
		{
			m_switches = new SwitchResult[numSwitches];
			for (int i = 0; i < numSwitches; i++)
			{
				m_switches[i] = new SwitchResult();
			}
		}
		...
		private bool ParseString(string srcString, SwitchForm[] switchForms)
		{
			...
			while (pos < len)
			{
				...
				int maxLen = kNoLen;
				for (int switchIndex = 0; switchIndex < m_switches.Length; switchIndex++)
				{
				...
				SwitchResult matchedSwitch = m_switches[matchedSwitchIndex];
		...
		private static bool IsItSwitchChar(char c)
			=> c == Parser.kSwitchID1 || c == Parser.kSwitchID2;
	}
